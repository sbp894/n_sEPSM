...
% Should use when SimData will actually be loaded as a 6-D structure. Else
% it will throw an error. 



%%
function parse_saved_data_for_all_conds_SNRenv(resultsDir,resultTxt)
...
sents= unique(cell2mat(csCell(:,2)));
...
figOutName=sprintf('CompareWithNF_%i__fixSPL_%i__CorrWindow_0add1mul_%i__nSent_avg_%i', ...
    CompareWithNF, ExpControlParams.fixSPL, ExpControlParams.winCorr0Add1Mul, length(sents));
...
OutputWeighted=repmat(struct('pModSN',[],'pModN',[],'SNRenv',[] ), length(noisetypes), length(sents), length(snrs));
...
for ntypeVar=1:length(noisetypes)
    for sentVar=1:length(sents)
        ...
        for snrVar=1:length(snrs)
            ...
            for ftypeVar=1:length(fibertypes)
                tempPowerModSN=[SimData(ntypeVar, 1, snrVar, sentVar, :, ftypeVar).PowerModSN]*SRweights(fibertypes(ftypeVar));
                tempPowerModSN(isnan(tempPowerModSN))=0;
                PowerModSN=PowerModSN+tempPowerModSN;
                
                tempPowerModN=[SimData(ntypeVar, 1, snrVar, sentVar, :, ftypeVar).PowerModN]*SRweights(fibertypes(ftypeVar));
                tempPowerModN(isnan(tempPowerModN))=0;
                PowerModN=PowerModN+tempPowerModN;
                
                tempSNRenv=[SimData(ntypeVar, 1, snrVar, sentVar, :, ftypeVar).SNRenv]*SRweights(fibertypes(ftypeVar));
                tempSNRenv(isnan(tempSNRenv))=0;
                SNRenv=SNRenv+tempSNRenv;
            end
            
            
            OutputWeighted(ntypeVar, sentVar, snrVar).pModSN=reshape(PowerModSN, length([SimData(1, 1, 1, 1, 1, 1).PowerModSN]), length(cfs))';
            OutputWeighted(ntypeVar, sentVar, snrVar).pModN=reshape(PowerModN, length([SimData(1, 1, 1, 1, 1).PowerModN]), length(cfs))';
            OutputWeighted(ntypeVar, sentVar, snrVar).SNRenv=reshape(SNRenv, length([SimData(1, 1, 1, 1, 1).SNRenv]), length(cfs))';
            
            
            OutputWeighted(ntypeVar, sentVar, snrVar).SNRenvSingle=nansum(nansum(OutputWeighted(ntypeVar, sentVar, snrVar).SNRenv.^2));
            
            %             subplot(4,3,subplotCount);
        ...
        %         saveas(gcf, [figOutDir figOutName '__' noisetypes{ntypeVar} 'sent' num2str(sentVar) '.bmp'], 'bmp');
        %         saveas(gcf, [figOutDir figOutName '__' noisetypes{ntypeVar} 'sent' num2str(sentVar)]);
    end
end
...
for ntypeVar=1:length(noisetypes)
        SNRenvs=20*log10(reshape([OutputWeighted(ntypeVar,:,:).SNRenvSingle], length(sents), length(snrs)));
%     SNRenvs=reshape([OutputWeighted(ntypeVar,:,:).SNRenvSingle], length(sents), length(snrs));
        SNRenvs(isinf(SNRenvs))=nan;
    errorbar(snrs, nanmean(SNRenvs),nanstd(SNRenvs), [ '-' markers(ntypeVar)],'linewidth',lw);
